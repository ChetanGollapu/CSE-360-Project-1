...
public class UserNameRecognizer {
	 ...
	 * @version 1.02		2025-01-XX	Modified to require first character be alphabetic and
	 * 									allow special characters (-, _, .) between alphanumeric
	 * 									characters only
	 * 
	 */
	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 0: 
				...
				// The first character must be alphabetic only (A-Z, a-z). Numbers are not
				// allowed as the first character in this modified version.
			...
			case 1: 
				...
				//	1: A-Z, a-z, 0-9 (UNChar) that transitions back to state 1
				//  2: special characters (-, _, .) that transition to state 2 

				
				// A-Z, a-z, 0-9 -> State 1 (UNChar characters)
				if ((currentChar >= 'A' && currentChar <= 'Z' ) ||		// Check for A-Z
						(currentChar >= 'a' && currentChar <= 'z' ) ||	// Check for a-z
			...
			case 2: 
				...
				// Only alphanumeric characters (UNChar) are allowed to follow special characters.
			...
			if (running) {
				...
				// Is the new state a final state?  Only state 1 is accepting.
		...
		case 0:
			...
			userNameRecognizerErrorMessage += "A UserName must start with an alphabetic character (A-Z, a-z).\n";
		...
		case 1:
			...
			if (userNameSize < 4) {
				...
				userNameRecognizerErrorMessage += 
					"A UserName may only contain alphanumeric characters (A-Z, a-z, 0-9) and special characters (-, _, .) between alphanumeric characters.\n";
				return userNameRecognizerErrorMessage;
			}
			else {
					// UserName is valid
		...
		case 2:
			...
			userNameRecognizerErrorMessage +=
				"A UserName cannot end with a special character (-, _, .). Special characters must be followed by alphanumeric characters (A-Z, a-z, 0-9).\n";
			return userNameRecognizerErrorMessage;

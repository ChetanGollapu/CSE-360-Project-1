...
* **@version** 2.01 2025-01-XX Modified to limit password length to maximum 32 characters
* with improved error messages for length validation.
*/
...
public static boolean foundTooLong = false; // New attribute to track if password exceeds 32 chars
...
* as the testing automation version. Modified to enforce a maximum length of 32 characters.
*
...
// Check if the password exceeds the maximum allowed length of 32 characters
if(input.length() > 32) {
passwordIndexofError = 32; // Point to the 33rd character (index 32)
foundTooLong = true;
return "*** Error *** The password is too long! Maximum length is 32 characters.";
}

// The input is not empty and within length limits, so we can access the first character
...
foundTooLong = false; // Reset the Boolean flag for length check
